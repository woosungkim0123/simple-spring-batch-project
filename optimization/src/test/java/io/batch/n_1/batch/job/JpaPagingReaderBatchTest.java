...
/**
 * JpaPagingItemReader를 사용해서 N+1 문제가 발생함. (batch size 적용 안되는걸 보여주는 예시)
 * 맨 밑 주석에서 로그 확인 가능
 */
@TestPropertySource(properties = "job.name=jpaPagingReaderBatch")
@SpringBootTest
class JpaPagingReaderBatchTest {
    ...
    @DisplayName("JpaPagingItemReader를 사용해서 N+1 문제가 발생한다는 걸 보여주는 테스트 코드")
    @Test

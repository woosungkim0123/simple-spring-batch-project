...
import optimization.reader.options.QuerydslNoOffsetOptions;
...
public class QuerydslZeroPagingItemReader<T> extends QuerydslPagingItemReader<T> {
    ...
    // pk가 꼭 id가 아닐 수 있기 때문에 options을 사용, 다른 필드도 정렬 조건에 사용할 수 있도록 함
    private QuerydslNoOffsetOptions<T> options;

    protected QuerydslZeroPagingItemReader() {
        super();
